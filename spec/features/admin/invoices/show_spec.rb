...
describe "Admin Invoices Index Page" do
  before :each do
    ...
    @discount1 = Discount.create!(percentage_discount: 15, quantity_threshold: 10, merchant_id: @m1.id)
    @discount2 = Discount.create!(percentage_discount: 25, quantity_threshold: 15, merchant_id: @m1.id)
  ...
  it "should have status as a select field that updates the invoices status" do
    ...
    # US 8
  it "should display the total revenue and total discounted revenue" do
    # save_and_open_page
    expect(page).to have_content("Total Revenue: $#{@i1.total_revenue}")
    expect(page).to have_content("Total Revenue After Discounts:")
    expect(page).to have_content(@i1.total_revenue_discounted)
  end
